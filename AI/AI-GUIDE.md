...
用户说“查看知识库状态”“知识库状态”“检查知识库”或要求查看整个知识库时，进入统一的只读状态模式。必须递归盘点 `Knowledge/`、`Projects/`、`Inbox/`、`Daily/` 和 `Archive/` 中的真实文件，排除仅凭顶层 README 判断为空；同时报告知识健康度、Git 工作区与分支状态、当前 `.kb-version`、模板最新版本和更新状态。存在新版时给出一次可选更新指令；存在未提交知识变更或本地领先提交时分别给出可选的提交或推送建议，但不得因状态检查自动修改、提交、推送或升级。普通的具体知识问答不附加这些状态提示。
