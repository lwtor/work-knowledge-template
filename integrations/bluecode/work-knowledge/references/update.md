...
9. After a successful update that leaves framework changes uncommitted, say explicitly that the framework is updated but Git has not recorded it, then ask: `框架已经更新完成，但这些变更尚未创建 Git 提交。是否要我先展示本次框架变更并创建本地 Git 提交？不会自动推送。`
