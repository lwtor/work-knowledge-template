...
For an explicit knowledge-base request, the marker root is mandatory and the current workspace must not replace it. Compare this installed `.skill-version` with the bluecode integration under the resolved personal root. If they differ, report that the installed Skill is stale and offer its repository installer; BlueCode must restart after installation.
