...
- 用户提供的路径默认解释为“保存位置”，不要求它本身为空。
- 如果该路径已经是个人知识库（包含 .kb-role=personal，或其 Git origin 指向目标个人仓库）：直接使用该路径。
- 如果该路径不是知识库：将最终目录解析为 该路径/work-knowledge；父目录中存在其他文件或项目不构成冲突。
- 如果用户提供的路径末级名称已经是 work-knowledge，则将其视为最终目录，不再追加子目录。
...
4. 按“本地路径规则”解析最终目录：最终目录不存在则 clone；已存在且 origin 相同则保留；只有最终目录非空且不是该知识库时才停止。不得因为父目录非空而报冲突，也不得覆盖未提交修改。
...
要求按本协议创建或接入，即授权创建最多一个 work-knowledge 仓库、clone 到用户指定或首次确认的目录并安装知识库 skill；除上述仅含 .kb-role 的初始化提交外，不授权上传其他文件、提交个人资料、删除内容或覆盖冲突目录。无法自动解决时停止，只给出一条最短操作说明。
