...
if ($repoRole -eq 'template') {
  ...
  foreach ($agentName in @('codex','bluecode','vbuddy')) {
    ...
    foreach ($marker in @('## ⚠️ 需要你确认','## ➡️ 可选的下一步','Put nothing after the action block','Never use bare','请直接回复','确认按上述预览写入知识库，不提交、不推送','确认将上述本地提交推送到已显示的远程分支')) { if (-not $interactionRules.Contains($marker)) { throw "$agentName 强提示规则缺少契约：$marker" } }
    ...
    foreach ($marker in @('Templates/','complete frontmatter','confidence: unverified','existing Git working-tree changes','确认按上述预览写入知识库，不提交、不推送','## ➡️ 可选的下一步','确认展示本次知识库变更并创建本地提交，不推送','separate explicit authorization')) { if (-not $ingestRules.Contains($marker)) { throw "$agentName 写入规则缺少收尾契约：$marker" } }
    ...
    foreach ($marker in @('raw.githubusercontent.com/lwtor/work-knowledge-template/main/UPDATE.md','temporary authority','preserve their union','Never use an old updater','no staged changes','framework allowlist','framework update has not started','Do not default to stashing','never use bare A/B','确认展示阻塞更新的现有修改','确认提交已展示的现有修改','## ➡️ 可选的下一步','确认展示框架变更并创建本地提交，不推送','确认生成目录迁移预览')) { if (-not $updateRules.Contains($marker)) { throw "$agentName 更新规则缺少跨版本契约：$marker" } }
  }
