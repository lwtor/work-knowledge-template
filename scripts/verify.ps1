...
foreach ($agentName in @('codex','bluecode','vbuddy')) {
  $required += @("integrations\$agentName\work-knowledge\references\layout.md","integrations\$agentName\work-knowledge\references\migration.md","integrations\$agentName\work-knowledge\references\interaction.md","integrations\$agentName\work-knowledge\references\content-routing.md")
}
...
if ($repoRole -eq 'template') {
  ...
  foreach ($agentName in @('codex','bluecode','vbuddy')) {
    ...
    foreach ($marker in @('name: work-knowledge','description:','references/layout.md','references/interaction.md','references/content-routing.md','references/ingest.md','references/query.md','references/project.md','references/maintenance.md','references/update.md','references/migration.md')) { if (-not $skillRules.Contains($marker)) { throw "$agentName Skill 入口缺少契约：$marker" } }
    ...
    foreach ($marker in @('AI 使用案例','Do not ask them to reconfirm','Do not split every case by default','原始记录','one shared case_id','记录类型','KSP')) { if (-not $contentRoutingRules.Contains($marker)) { throw "$agentName 内容分类规则缺少契约：$marker" } }
...
if ((Test-Path -LiteralPath $roleFile -PathType Leaf) -and ((Get-Content -Raw -LiteralPath $roleFile).Trim() -eq 'personal')) {
 if ($Agent -in @('Codex','All')) {
  ...
  foreach ($relative in @('SKILL.md','references\layout.md','references\interaction.md','references\content-routing.md','references\ingest.md','references\query.md','references\project.md','references\maintenance.md','references\update.md','references\migration.md')) {
