...
$required += @('Templates\需求主页.md','Templates\案例复盘.md','Templates\案例原始对话.md','Templates\快速记录.md','Templates\文档学习笔记.md','Vault\.obsidian\appearance.json','Vault\.obsidian\snippets\work-knowledge-vault.css','integrations\shared\work-knowledge\references\interaction.md','integrations\shared\work-knowledge\references\content-routing.md','scripts\sync-agent-interaction.py')
foreach ($agentName in @('codex','bluecode','vbuddy')) {
  $required += @("integrations\$agentName\work-knowledge\references\layout.md","integrations\$agentName\work-knowledge\references\migration.md","integrations\$agentName\work-knowledge\references\interaction.md","integrations\$agentName\work-knowledge\references\content-routing.md","integrations\$agentName\work-knowledge\.skill-version")
}
...
if ($repoRole -eq 'template') {
  ...
  $sharedInteraction = Join-Path $root 'integrations\shared\work-knowledge\references\interaction.md'
  $sharedInteractionHash = (Get-FileHash -LiteralPath $sharedInteraction -Algorithm SHA256).Hash
  $sharedContentRouting = Join-Path $root 'integrations\shared\work-knowledge\references\content-routing.md'
  $sharedContentRoutingHash = (Get-FileHash -LiteralPath $sharedContentRouting -Algorithm SHA256).Hash
  foreach ($agentName in @('codex','bluecode','vbuddy')) {
    $skillRules = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $root "integrations\$agentName\work-knowledge\SKILL.md")
    foreach ($marker in @('name: work-knowledge','description:','references/layout.md','references/interaction.md','references/content-routing.md','.skill-version','references/ingest.md','references/query.md','references/project.md','references/maintenance.md','references/update.md','references/migration.md')) { if (-not $skillRules.Contains($marker)) { throw "$agentName Skill 入口缺少契约：$marker" } }
    ...
    if ((Get-FileHash -LiteralPath (Join-Path $root "integrations\$agentName\work-knowledge\references\interaction.md") -Algorithm SHA256).Hash -ne $sharedInteractionHash) { throw "$agentName 交互规则与共享权威文件不一致；请运行 python scripts/sync-agent-interaction.py" }
    $contentRoutingRules = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $root "integrations\$agentName\work-knowledge\references\content-routing.md")
    if ((Get-FileHash -LiteralPath (Join-Path $root "integrations\$agentName\work-knowledge\references\content-routing.md") -Algorithm SHA256).Hash -ne $sharedContentRoutingHash) { throw "$agentName 内容分类规则与共享权威文件不一致；请运行 python scripts/sync-agent-interaction.py" }
    foreach ($marker in @('## Destination gate','kb-target-check.py','AI 使用案例','Do not ask them to reconfirm','Do not split every case by default','原始记录','one shared case_id','记录类型','KSP')) { if (-not $contentRoutingRules.Contains($marker)) { throw "$agentName 内容分类规则缺少契约：$marker" } }
    foreach ($marker in @('## ⚠️ 需要你确认','## ➡️ 可选的下一步','Reply `0`','回复数字','most recent action block','recompute all currently applicable actions','Choosing one item does not dismiss the others','确认按上述预览写入知识库，不提交、不推送','确认将上述本地提交推送到已显示的远程分支')) { if (-not $interactionRules.Contains($marker)) { throw "$agentName 强提示规则缺少契约：$marker" } }
    ...
    foreach ($marker in @('recursively enumerate','skill_version','skill_rules_sha256','重新安装当前 Agent 的知识库 Skill','.kb-version','raw.githubusercontent.com/lwtor/work-knowledge-template/main/.kb-version','Do not inspect only the first directory level','current branch','knowledge health','查看知识库状态','Recompute the pending-action queue','Choosing one item does not dismiss the others')) { if (-not $queryRules.Contains($marker)) { throw "$agentName 查询规则缺少状态契约：$marker" } }
    ...
    foreach ($marker in @('references/content-routing.md','kb-target-check.py','知识库根目录','Templates/','complete frontmatter','confidence: unverified','记录类型','目标位置','记录重点','explicitly requested linked case transcript','existing Git working-tree changes','numbered required-confirmation block','确认按上述预览写入知识库，不提交、不推送','recomputed optional-action queue','Include every other still-applicable action too','确认展示本次知识库变更并创建本地提交，不推送','separate explicit authorization')) { if (-not $ingestRules.Contains($marker)) { throw "$agentName 写入规则缺少收尾契约：$marker" } }
    $maintenanceRules = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $root "integrations\$agentName\work-knowledge\references\maintenance.md")
    foreach ($marker in @('确认按上述预览归档指定笔记，不提交、不推送','确认删除上述指定文件，不提交、不推送','确认按上述预览合并指定笔记，不提交、不推送','确认按上述预览移动指定文件，不提交、不推送')) { if (-not $maintenanceRules.Contains($marker)) { throw "$agentName 维护规则缺少确认契约：$marker" } }
...
function Assert-SkillMarker([string]$AgentName, [string]$MarkerPath) {
  $markerText = Get-Content -Raw -LiteralPath $MarkerPath
  $expectedInfo = @(python (Join-Path $root 'scripts\kb-skill-info.py') --root $root --agent $AgentName)
  if ($LASTEXITCODE -ne 0) { throw "无法计算 $AgentName Skill 版本信息。" }
  foreach ($line in $expectedInfo) { if (-not $markerText.Contains($line)) { throw "$AgentName Skill marker 版本或规则哈希不一致：$line" } }
  if (-not $markerText.Contains('installed_at=')) { throw "$AgentName Skill marker 缺少 installed_at。" }
}
if ((Test-Path -LiteralPath $roleFile -PathType Leaf) -and ((Get-Content -Raw -LiteralPath $roleFile).Trim() -eq 'personal')) {
 if ($Agent -in @('Codex','All')) {
  ...
  Assert-SkillMarker 'codex' $marker
  foreach ($relative in @('SKILL.md','.skill-version','references\layout.md','references\interaction.md','references\content-routing.md','references\ingest.md','references\query.md','references\project.md','references\maintenance.md','references\update.md','references\migration.md')) {
 ...
 if ($Agent -in @('BlueCode','All')) {
  ...
  if (Test-Path -LiteralPath $bluecodeInstalled) {
    ...
    Assert-SkillMarker 'bluecode' $bluecodeMarker
    foreach ($relative in @('SKILL.md','.skill-version','references\layout.md','references\interaction.md','references\content-routing.md','references\ingest.md','references\query.md','references\project.md','references\maintenance.md','references\update.md','references\migration.md')) {
 ...
 if ($Agent -in @('Vbuddy','All')) {
  ...
  if (Test-Path -LiteralPath $vbuddyInstalled) {
    ...
    Assert-SkillMarker 'vbuddy' $vbuddyMarker
    foreach ($relative in @('SKILL.md','.skill-version','references\layout.md','references\interaction.md','references\content-routing.md','references\ingest.md','references\query.md','references\project.md','references\maintenance.md','references\update.md','references\migration.md')) {
