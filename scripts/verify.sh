...
required=(
  ...
  "Templates/需求主页.md" "Templates/案例复盘.md" "Templates/快速记录.md" "Templates/文档学习笔记.md"
...
if [[ -f "$root/.kb-role" ]] && [[ "$(tr -d '\r\n' < "$root/.kb-role")" == "template" ]]; then
  ...
  for agent_name in codex bluecode vbuddy; do
    ...
    for marker in '## ⚠️ 需要你确认' '## ➡️ 可选的下一步' 'Put nothing after the action block' '请直接回复'; do
    ...
    for marker in 'Templates/' 'complete frontmatter' 'confidence: unverified' 'existing Git working-tree changes' '## ➡️ 可选的下一步' '确认展示本次知识库变更并创建本地提交，不推送' 'separate explicit authorization'; do
    ...
    for marker in 'raw.githubusercontent.com/lwtor/work-knowledge-template/main/UPDATE.md' 'temporary authority' 'preserve their union' 'Never use an old updater' 'no staged changes' 'framework allowlist' 'framework update has not started' 'Do not default to stashing' '## ➡️ 可选的下一步' '确认展示框架变更并创建本地提交，不推送' '确认生成目录迁移预览'; do
...
if [[ -f "$root/.kb-role" ]] && [[ "$(tr -d '\r\n' < "$root/.kb-role")" == "personal" ]]; then
 ...
 if [[ "$agent" == "bluecode" || "$agent" == "all" ]]; then
  ...
  if [[ -e "$bdir" ]]; then
    ...
    for relative in SKILL.md references/layout.md references/interaction.md references/ingest.md references/query.md references/project.md references/maintenance.md references/update.md references/migration.md; do cmp -s "$root/integrations/bluecode/work-knowledge/$relative" "$bdir/$relative" || { echo "BlueCode skill 与仓库版本不一致：$relative" >&2; exit 1; }; done
 ...
 if [[ "$agent" == "vbuddy" || "$agent" == "all" ]]; then
  ...
  if [[ -e "$vdir" ]]; then
    ...
    for relative in SKILL.md references/layout.md references/interaction.md references/ingest.md references/query.md references/project.md references/maintenance.md references/update.md references/migration.md; do cmp -s "$root/integrations/vbuddy/work-knowledge/$relative" "$vdir/$relative" || { echo "vBuddy skill 与仓库版本不一致：$relative" >&2; exit 1; }; done
